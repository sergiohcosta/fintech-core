...
**Limitação conhecida — 1 imagem = 1 transação, sempre:** `LlmReceiptExtractionDTO` é plano por design (Fase 1 = comprovante único). Print de **extrato completo** (várias transações na mesma imagem) não é detectado como fora de escopo — hoje o modelo "escolhe" uma linha e o guarda-corpo (`amount>0`) deixa passar, descartando o resto silenciosamente. Guarda-corpo de recusa explícita: #193. Suporte real a multi-transação por imagem: roadmap Fase 3, #194.
