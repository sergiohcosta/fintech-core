...
**`PdfTextExtractor` (Fase 3, `pdf_text_v1`, Apache PDFBox):** `supports()` reconhece **qualquer** PDF pelo magic number (`%PDF-`) — a distinção "tem texto ou não" só é possível dentro de `extract()`, depois que o PDFBox abre o documento. **PDF escaneado (sem texto extraível, fatia 3):** cada página é rasterizada (Apache PDFBox `PDFRenderer`, PNG) e delegada ao `VisionExtractor` já existente — reaproveita fallback Gemini/Ollama e detecção multi-transação (#194) sem duplicar lógica de visão. Guard-rails: fail-fast em `import.pdf-scanned.max-pages` (default 10, checado ANTES de qualquer chamada de IA) e all-or-nothing (falha de uma página derruba o batch inteiro). `sourceType=PDF_SCANNED` (enum e colunas de proveniência V28 já existiam sem uso) e `requiresReview=true` forçado em toda linha — mesmo staged rollout do caminho de extrato do #194, zero dado de produção sobre acurácia em página rasterizada. Sem templates bancários para este caminho (mesmo princípio de não construir reconhecimento específico sem volume real). **`sourceType` em batch `FAILED`:** `TransactionExtractor.sourceType()` é fixo por extrator (`PDF_TEXT`) — não cobre falha ocorrida DEPOIS de decidir o sub-caminho escaneado (limite de páginas, página ilegível). `ScannedPdfExtractionException` (subtipo de `ExtractionException`) carrega esse sinal; `ImportService.createFromFile` checa o tipo antes de gravar, senão todo `FAILED` deste caminho gravaria `PDF_TEXT` — o oposto do que a proveniência (V28) existe pra rastrear. Reconhecimento de transação (PDF **com** texto) por **heurística de linha**: uma linha vira transação só se casar um padrão de **data** (`DD/MM/YYYY`, `DD/MM/YY` ou `YYYY-MM-DD`) **e** um **valor monetário** (`1.234,56`/`1234.56`, sinal e `R$` opcionais) na mesma linha — sem os dois, a linha simplesmente não é transação (não é erro, é ausência de sinal: cabeçalho, rodapé, linha de saldo). `amount`/`transaction_date` confiança `1.0` (padrão bateu); `direction`/`description` confiança `0.7` (inferência por sinal/posição, mesma régua do CSV). Palavra-chave `débito`/`crédito` na linha tem prioridade sobre o sinal do valor. **Registry de templates (Fase 3, fatia 2):** antes da heurística genérica, o `PdfTextExtractor` tenta templates por banco (`PdfBankTemplate`, mesmo padrão de lista de beans ordenada do `VisionModelClient`) — detecção por CNPJ da instituição + rótulo de seção conhecido no texto extraído (nunca por nome de arquivo). Hoje: `ItauFaturaTemplate` (`itau_fatura_v1`, datas sem ano inferidas da data de vencimento, exclui "Compras parceladas - próximas faturas" do mês corrente) e `NubankExtratoTemplate` (`nubank_extrato_v1`, state machine linha a linha, direção pela seção "Total de entradas"/"Total de saídas" corrente). Nenhum template bate → heurística genérica de linha (fatia 1), inalterada. `extractor_used` grava o `templateId()` quando um template processa.
