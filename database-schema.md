...
| V32 | `version BIGINT NOT NULL DEFAULT 0` em `staged_transactions` — optimistic locking (JPA `@Version`). Impede promoção dupla em commits concorrentes: a segunda requisição recebe `ObjectOptimisticLockingFailureException` → 409 Conflict (#222) |
| V33 | `ENABLE`/`FORCE ROW LEVEL SECURITY` + `CREATE POLICY tenant_isolation` em `transactions` — PoC de RLS como defesa em profundidade (#116, ADR-006). `USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::uuid)`; `TenantRlsAspect` seta `SET LOCAL app.tenant_id` em `TransactionService.*` e `InvoiceService.pay` antes de cada operação. **Pré-requisito operacional:** exige role Postgres sem `SUPERUSER`/`BYPASSRLS` conectando como runtime da app — superuser sempre ignora RLS, mesmo com `FORCE` (achado real do PoC: o `admin` do `docker-compose.yml` é superuser por padrão da imagem oficial `postgres`; role `fintech_app` criado via `.docker/postgres-init/01-app-role.sql`, `spring.datasource.*` local aponta pra ele, `spring.flyway.*` continua com `admin`). Homelab (dev/hmg/prod): confirmado que os 3 roles são owners sem `BYPASSRLS` — `FORCE` já basta lá, sem mudança de infra (ver ADR-006) |
